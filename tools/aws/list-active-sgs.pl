...
while (<TAGS>) {
   if (/\w+\s+\w+\s+(\S+)\s+\w+\s+(\S+)/){
      my $instance_id = $1; #essentially maps the last id associated with the tag name
      my $tag = $2;
#      my $tag_prefix = $1 if ($tag=~/(\w+-\w{3})/);
      $instance_tags{$tag} = $instance_id;
#   print "adding $tag"
   }
}
close (TAGS);
...
open (SG, "aws ec2 describe-security-groups --region $region --query \"SecurityGroups[*].[GroupId,GroupName,FromPort,ToPort,IpPermissions]\" --filters 'Name=ip-permission.cidr,Values=128.177.173.6*' --output text|");
...
foreach my $tag (keys %instance_tags) {
  foreach my $security_group_id (@{$instance_security_groups{$instance_tags{$tag}}}) {
        foreach my $sg_entry (@{$security_groups{$security_group_id}}) {
            #print "\n @{$sg_entry}[1] ";
            push (@{ $tmp{ @{$sg_entry}[1]} }, $tag);
            #print "adding $tag onto @{$sg_entry}[1] "
        }
  }
}
